...
(function ($) {
  ...
  function select_or_other_check_and_show(ele, page_init) {
    ...
    if (jQuery(ele).find(".select-or-other-select option:selected[value=select_or_other], .select-or-other-select:checked[value=select_or_other]").length) {
      jQuery(ele).find(".select-or-other-other").parents("div.control-group").first().show(speed, function() {
        if(!page_init) {
      ...
      jQuery(ele).find(".select-or-other-other").parents("div.control-group").first().hide(speed);
      if (page_init)
      {
